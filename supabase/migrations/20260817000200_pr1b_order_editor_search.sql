...
select 'garments', 'Prendas', profile.id, profile.id
from (select id from public.profiles where role = 'super_admin' and is_active order by created_at limit 1) profile
where not exists (select 1 from public.catalog_sections where code = 'garments');

drop function public.get_order_board();

...
begin
  ...
  if normalized_code is null then
    ...
    if found then
      update public.price_products
      set group_name=p_group, name=btrim(p_name), unit=p_unit, price=p_price, is_active=coalesce(p_is_active,is_active), updated_by=actor.id, updated_at=now()
      where id=product_id;
      return product_id;
    end if;
  end if;
...
begin
  ...
  for row in select value from jsonb_array_elements(p_rows) loop
    ...
    if code is not null then
      ...
      from public.price_products
      where lower(btrim(name)) = lower(btrim(row->>'name'))
      order by updated_at desc, id
      limit 1;
    end if;

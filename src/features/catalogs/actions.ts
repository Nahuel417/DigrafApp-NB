...
import { catalogItemIdSchema, catalogItemSchema, catalogProductSchema, productCatalogKindLabels, renameCatalogItemSchema } from "./schemas";
...
export async function createCatalogProductAction(_previous: CatalogActionState, formData: FormData): Promise<CatalogActionState> {
  const parsed = catalogProductSchema.safeParse({ kind: formValue(formData, "kind"), name: formValue(formData, "name") });
  ...
  const { error } = await supabase.rpc("create_catalog_product_without_category", { target_section_id: sectionId, target_kind: parsed.data.kind, target_name: parsed.data.name });
  if (error) return mutationResult("error", catalogErrorMessage(error.message));
  revalidatePath("/catalogs");

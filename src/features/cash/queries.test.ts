...
describe("cash summary mapping", () => {
  ...
  it("maps a genuine untouched day to zero and empty lists", () => {
    const summary = mapCashSummary({ ...base, categories: [], current_balance: 0, opening_balance: 0, movements: [] });
  ...
  it.each([
    ...
    expect(() => mapCashSummary({ ...base, categories: [], current_balance: 0, opening_balance: 0, movements: [], ...override } as never)).toThrow("La respuesta de caja");
  });
